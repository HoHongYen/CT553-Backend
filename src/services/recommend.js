...
class RecommendService {
    ...
    static async getRecommend(accountId) {
        ...
        for (let i = 0; i < n_accounts; i++) {
            ...
            for (let j = 0; j < n_accounts; j++) {
                ...
                if (i !== j) {
                    ...
                    sim[i][j] = sum1 / (Math.sqrt(sum2) * Math.sqrt(sum3));
                }
            }
        }
        console.log('sim ', sim);
        ...
        for (let i = 0; i < n_products; i++) {
            for (let j = 0; j < n_accounts; j++) {
                if (a[i][j] === -1)
                    rating[i][j] = Math.min(5, rating[i][j] + mean[j]);
            }
        }
        console.log('rating ', rating);
